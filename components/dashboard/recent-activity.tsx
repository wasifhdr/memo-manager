    ...
    <ul className="flex flex-col divide-y divide-(--color-sand)">
      {items.map((a) => (
        <li key={a.id} className="flex items-baseline justify-between gap-3 py-2.5 first:pt-0 last:pb-0">
          <span className="min-w-0 truncate text-[0.8125rem] text-(--color-ink)">
            {a.actorName ? <span className="font-bold">{a.actorName}: </span> : null}
            {a.description}
          </span>
          <span className="shrink-0 font-mono-nums text-[0.6875rem] text-(--color-ink)/50">{formatDateTime(a.createdAt)}</span>
        </li>
      ))}
    </ul>
  )
