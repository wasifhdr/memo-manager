...
export async function userDashboard(ctx: TenantContext) {
  ...
  const [awaitingMyAction, submittedByMe, recentlyCompleted] = await Promise.all([
    listInbox(ctx, { pageSize: 5 }),
    listMyMemos(ctx, { pageSize: 5 }),
    listCompleted(ctx, { pageSize: 5 }),
  ])

  const inboxByAction = await db.select({
    requiredAction: workflowSteps.requiredAction, n: count(),
  }).from(memos)
  ...
  const [{ n: urgentMemos }] = await db.select({ n: count() }).from(memos)
    .where(and(
  ...
  const statusRows = await db.select({ status: memos.status, n: count() }).from(memos)
